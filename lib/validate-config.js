...
function validateConfig(rawConfig, configPath) {
  ...
  function validatePropertyType(propertyName, fn, typeDescription) {
  ...
  validatePropertyType(
    'stylesheets',
    isArrayOf(x => {
      return _.isString(x) && (isAbsolutePath(x) || isAbsoluteUrl(x));
    }),
    'array of absolute file paths or absolute URLs'
  );
