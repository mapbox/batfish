...
function validateConfig(config, configPath) {
  ...
  // Ensure unprocessedPageFiles is an array of relative paths.
  if (configWithDefaults.unprocessedPageFiles !== undefined) {
    if (!Array.isArray(configWithDefaults.unprocessedPageFiles)) {
      configWithDefaults.unprocessedPageFiles = [
        configWithDefaults.unprocessedPageFiles
      ];
    }
    configWithDefaults.unprocessedPageFiles.forEach(glob => {
      if (path.isAbsolute(glob)) {
        throw new Error(
          `unprocessedPageFiles globs should be relative to the pagesDirectory, not absolute paths`
        );
      }
    });
  }
  ...
  if (
    configWithDefaults.applicationWrapperPath &&
    !isAbsolutePath(configWithDefaults.applicationWrapperPath)
  ) {
    throw new Error('applicationWrapperPath must be an absolute path');
  }
  if (
  ...
  if (
    configWithDefaults.dataSelectors &&
    (!_.isPlainObject(configWithDefaults.dataSelectors) ||
      !_.every(configWithDefaults.dataSelectors, _.isFunction))
  ) {
    throw new Error(
      'dataSelectors must be an object whose property values are functions'
    );
  }
