...
function inlineCss(htmlRoot, cssPath, options) {
  ...
  const processPage = htmlPath => {
    ...
    return new Promise(resolve => {
      if (options.verbose) {
        timelog(`Starting to inline CSS for ${htmlPath}`);
      }
      workers(htmlPath, cssPath, (error, output) => {
        ...
        if (options.verbose) {
          timelog(`Successfully inlined CSS for ${htmlPath}`);
        }
        resolve();
      });
    });
  ...
  return globby(pagesGlob)
    .then(htmlPaths => {
      ...
      timelog(`Inlining CSS in ${htmlPaths.length} files`);
