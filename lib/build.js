...
function build(options) {
  ...
  const assetOutputDirectory = path.join(options.outputDirectory, 'assets');

  const configOptions = _.assign(
    _.pick(options, ['debug', 'entry', 'sourceDirectory', 'vendor']),
    {
  ...
  const buildClient = pify(webpack)(clientConfig).then(stats => {
    return pify(fs.writeFile)(
      path.join(options.outputDirectory, 'stats.json'),
      JSON.stringify(stats.toJson())
    );
  });
  const buildStatic = pify(webpack)(staticConfig);

  return del(options.outputDirectory)
