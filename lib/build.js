...
function build(rawConfig, projectDirectory) {
  ...
  batfishLog.log('Starting the Webpack bundling');
  ...
  return start
    ...
    .then(() => {
      batfishLog.log('Creating the client bundle');
      return buildClient();
    })
    .then(() => {
      batfishLog.log('Creating the static-page-rendering bundle');
      return buildStatic();
    })
    .then(() => {
      batfishLog.log('Copying files');
    ...
    .then(rawAssets => {
      ...
      batfishLog.log('Starting static HTML build');
      ...
      try {
        ...
        batfishLog.log(
          `${chalk.red.bold('Error parsing static-render-pages.js')}`
        );
        throw requireError;
      }
    })
    ...
    .then(() => {
      if (batfishConfig.siteOrigin) {
        batfishLog.log('Building the sitemap');
    ...
    .then(() => {
      batfishLog.log('Successful static HTML build');
    });
}
