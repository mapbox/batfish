...
const babelPresets = [
  // Let Webpack handle module syntax
...
function createWebpackConfigBase(batfishConfig) {
  ...
  return createBatfishContext(batfishConfig).then(batfishContextPath => {
    ...
    jsxtremeMarkdownOptions.modules.push(
      ...
      `const prefixUrlAbsolute = require('@mapbox/batfish/modules/prefix-url').prefixUrlAbsolute;`
    );
    jsxtremeMarkdownOptions.modules.push(
      `const routeTo = require('@mapbox/batfish/modules/route-to').routeTo;`,
      `const routeToPrefixed = require('@mapbox/batfish/modules/route-to').routeToPrefixed;`
    );
    ...
    const config = {
      ...
      resolve: {
        alias: {
          ...
          'batfish-internal/wrapper': batfishConfig.wrapperPath,
          // Not necessary for dependents, but necessary for examples
          '@mapbox/batfish/modules': path.join(__dirname, '../modules')
        }
      },
      module: {
        rules: [
      ...
      stats: batfishConfig.verbose ? 'normal' : 'errors-only',
